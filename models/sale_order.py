...
class SaleOrder(models.Model):
    ...
    new_created = fields.Boolean(default=True)
    new_chat = fields.Boolean(default=False)
    paidout = fields.Boolean(default=False)
    finish = fields.Boolean(default=False) # For determing if so is completly finish
    ...
    def write(self, values):
        if values.get('commitment_date'):
            ...
            for purchase_order in purchase_orders:
                purchase_order.commitment_date = values.get('commitment_date')
                purchase_order.date_notification = True
                self.env['bus.bus'].sendone(
                    self._cr.dbname + '_' + str(purchase_order.partner_id.id),
                    {'type': 'sale_order_notification', 'action':'new_date',"date":purchase_order.commitment_date ,"Po":purchase_order.id})
...
class SaleOrderLine(models.Model):
    ...
    def _purchase_service_prepare_order_values(self, supplierinfo):
        ...
        return {
            ...
            'new_created' : self.order_id.new_created,
            'new_chat' : self.order_id.new_chat,
            'anonimus_author' : self.order_id.anonimus_author,
            'anonimus' : self.order_id.anonimus, # For determing So anonymus
